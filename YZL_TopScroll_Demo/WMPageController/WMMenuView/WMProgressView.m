...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat currentWidth = currentFrame.size.width * self.widthRatio;
    int nextIndex = index + 1 < self.itemFrames.count ? index + 1 : index;
    CGFloat nextWidth = [self.itemFrames[nextIndex] CGRectValue].size.width * self.widthRatio;
    CGFloat height = self.frame.size.height;
    CGFloat constY = height / 2;
    CGFloat currentX = currentFrame.size.width * (1 - self.widthRatio) / 2 + currentFrame.origin.x;
    CGFloat nextX = currentFrame.size.width * (1 - self.widthRatio) / 2 + [self.itemFrames[nextIndex] CGRectValue].origin.x;
